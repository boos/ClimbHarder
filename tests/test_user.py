def test_cannot_create_user_without_username_email_password():
    pass


def test_cannot_create_user_without_username():
    pass


def test_cannot_create_user_without_email():
    pass


def test_cannot_create_user_without_password():
    pass


def test_cannot_create_user_existing_username():
    pass


def test_cannot_create_user_existing_email():
    pass


def test_can_successfully_create_user():
    pass


def test_cannot_delete_other_user_if_not_admin():
    pass


def test_can_delete_user_self()


pass


def test_can_get_my_user_details():
    pass


def test_cannot_update_other_user_details():
    pass


def test_update_user_details_if_self():
    pass


def test_cannot_get_other_user_details_if_not_public():
    pass
